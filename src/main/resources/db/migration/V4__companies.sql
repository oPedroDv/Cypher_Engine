...
(
    ...
    cnpj_status          VARCHAR(32)  NOT NULL DEFAULT 'DESCONHECIDO',
    ...
    CONSTRAINT chk_companies_cnpj_status CHECK (
        cnpj_status IN ('ATIVA', 'SUSPENSA', 'INAPTA', 'BAIXADA', 'NULA', 'DESCONHECIDO')
        )
);
...
CREATE INDEX idx_companies_status_checked ON companies (status_checked_at ASC NULLS FIRST)
    WHERE status_checked_at IS NULL OR cnpj_status != 'BAIXADA';
